...
import { mockAddOrganization, mockAddOrganizationRequest } from '@/tests/presentation/mocks/organization-mocks'
...
describe('AddOrganizationController', () => {
  it('should call validator with correct value', async () => {
    ...
    await sut.handle(mockAddOrganizationRequest())
    expect(validatorSpy).toHaveBeenCalledWith(mockAddOrganizationRequest().body)
  })
  it('should return 400 if validation fails', async () => {
    const { sut, validatorStub } = makeSut()
    jest.spyOn(validatorStub, 'validate').mockReturnValueOnce(new Error())
    const httpResponse = await sut.handle(mockAddOrganizationRequest())
  ...
  it('should call addOrganization with correct value', async () => {
    ...
    await sut.handle(mockAddOrganizationRequest())
    expect(addSpy).toHaveBeenCalledWith(mockAddOrganizationRequest().body)
  })
